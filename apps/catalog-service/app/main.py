...
def upsert_song(payload: UpsertSongRequest, db: Session = Depends(db_dep)) -> Song:
    song = db.scalar(
        select(Song).where(
...
def add_user_song(payload: AddUserSongRequest, db: Session = Depends(db_dep)) -> dict[str, str]:
    existing = db.scalar(
...
def user_library(user_id: str, db: Session = Depends(db_dep)) -> dict[str, list[dict]]:
    rows = db.execute(
        select(Song)
...
def get_song(song_id: str, db: Session = Depends(db_dep)) -> Song:
...
def get_song_by_source(source_provider: str, source_id: str, db: Session = Depends(db_dep)) -> Song:
    song = db.scalar(
