...
def stream_url(song_id: str, user_id: str = Query(...), db: Session = Depends(db_dep)):
    owns = db.scalar(
    ...
    return {"stream_url": f"{PUBLIC_STREAM_BASE}/public/stream/{song_id}?user_id={user_id}"}


@app.get("/public/stream/{song_id}")
def public_stream(song_id: str, user_id: str, request: Request, db: Session = Depends(db_dep)):
    owns = db.scalar(
        select(UserSong).where(and_(UserSong.user_id == user_id, UserSong.song_id == song_id))
    )
    if owns is None:
